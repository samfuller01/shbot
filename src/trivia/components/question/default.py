...
class TriviaGameComponentQuestionDefault(TriviaGameComponent):

    async def __init__(self, parent, client):
        super(TriviaGameComponentQuestionDefault, self).__init__(parent=parent, client=client)
        # ...
    ...
    async def Setup(self):
        ...
        if self.state != "finished":
            ...
            if self.state != "finished":
                ...
                _num_failed = self.parent.get("num_failed")

                self.parent.set("num_failed", _num_failed + 1)
                if _num_failed + 1 == 15:
    ...
    def build_leaderboard(self):
        ...
        s = ""
        d = self.parent.get("scores")
        k = list(d.keys())
        k.sort(key=lambda j: d[j], reverse=True)
        for x in k:
            _points = self.parent.get("scores")[x]
