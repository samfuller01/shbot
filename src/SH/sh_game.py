...
class SHGame (Game):
    ...
    async def __init__(self, players=None, client=None, category=None, flags=None, context=None):
        super(SHGame, self).__init__(players=players, client=client, category=category, flags=flags, context=context)
        #
        #
        ...
        #
        # self.load_component_names(preset=flags["preset"]) #TODO
        ...
        #_config    = DEFAULT_PRESETS[self.size] if flags["preset"] == None or flags["preset"] == "default" else "{root}/{pre}.json".format(root=PRESET_PATH, pre=flags["preset"])
        self.board = await SHBoard(parent=self, client=self.client, size=self.size, context=context)
        self.deck  = SHDeck()
        #
    ...
    async def enact_policy(self, policy_type, was_topdecked=False, fire_event=True):
        ...
        # post_policy. If it was played, then load the policy_power component.
        #
        if was_topdecked:
