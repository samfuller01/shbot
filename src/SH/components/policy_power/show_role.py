...
class SHGameComponentPolicy_powerShow_role (SHGameComponent):

    async def __init__(self, parent, client):
        super(SHGameComponentPolicy_powerShow_role, self).__init__(parent=parent, client=client)
        # ...

    async def Setup(self):
        _msg = "You must show your role to a player."
    ...
    async def Handle(self, context):
        ...
        # if they added a reaction
            ...
            if _target != None and self.is_legal_pick(_target):
                ...
                _role = self.parent.s_seats[_pres]["role"]
                _msg = "You show your role to player " + self.parent.s_seats[_target]["name"] + "."
                await self.parent.message_seat(_pres, content=_msg)

                _msg2 = "You see the role of player " + self.parent.s_seats[_pres]["name"] + " and see: " + str(_role)
                await self.parent.message_seat(_target, content=_msg2)

                await self.parent.message_main(content="President " + self.parent.s_seats[_pres]["name"] + " shows their role to " + 
